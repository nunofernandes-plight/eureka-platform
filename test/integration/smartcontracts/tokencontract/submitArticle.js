...
test('submit article', async t => {
	...
	const authors = await getAuthors(EurekaPlatformContract, articleHashHex, accounts[1]);
	...
	const linkedArticles = await getLinkedArticles(EurekaPlatformContract, articleHashHex, accounts[1]);
	t.deepEqual(linkedArticles[2].substring(2), article.linkedArticles[2], 'should be ' + article.linkedArticles[2]);

	const url = await getUrl(EurekaPlatformContract, articleHashHex, accounts[1]);
